...
export type PlaylistGenerationConstraints = {
  languageHint?: string;
...
export function extractArtistNamesFromRequest(request: string): string[] {
...
export async function buildArtistDrivenPlaylist(
  request: string,
): Promise<MatchedSong[]> {
...
async function findBestDeezerMatch(
  suggestion: string,
  ...
  for (const track of candidates) {
    ...
    const artistContains =
      ...
      (normTrackArtist.includes(normArtist) ||
        ...
        : 0.0;
    const combinedScore =
      titleJaccard * 0.5 + artistJaccard * 0.3 + titleContains + artistContains;
...
export async function generateSongsFromPrompt(
  request: string,
    ...
    messages: [
      ...
      },
      {
        ...
        content:
          ...
          (languageHint ? `Bevorzugte Songsprache: ${languageHint}. ` : "") +
...
export async function generatePlaylistProfileFromSongs(
  sourceSongs: string[],
