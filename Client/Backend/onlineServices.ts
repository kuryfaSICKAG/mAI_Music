...
export async function sendPlaylist(
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "Keine Verbindung zum Server. Bitte zuerst connectToServer() ausführen.",
    };
  }
...
export async function getReceivedPlaylists(
  ...
  const url = joinUrl(
    base,
    `/playlist/received/${encodeURIComponent(username)}`,
  );
...
export async function getPublicPlaylistDetail(
  ...
  | {
      ...
      detail: {
        username: string;
        playlist: { name: string; songs: string[]; status: Status };
      };
    }
  | { ok: false; error: string }
> {
  ...
  const url = joinUrl(
    base,
    `/playlist/public/${encodeURIComponent(username)}/${encodeURIComponent(name)}`,
  );
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ok: false,
        error: `HTTP ${res.status}: ${text || "Öffentliche Playlist nicht gefunden"}`,
      };
    }
    const detail = await safeJson(res, {
      username,
      playlist: { name, songs: [] as string[], status: "public" as Status },
    });
...
> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ok: false,
        error: `HTTP ${res.status}: ${text || "Song-Details nicht verfügbar"}`,
      };
    }
